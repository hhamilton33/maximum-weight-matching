...
def solve_lp(n,seed):
    ...
    for k in range(n):
        gm.addConstr(quicksum(x[i,k] for i in range(k) if w[i][k] >= 1)+quicksum(x[k,j] for j in range(k+1,n) if w[k][j] >= 1)<=1)
    ...
    for i in range(n):
        for j in range(n):
            for k in range(n):
                if i<j:
                    if j<k:
                        if w[i][j] >= 1 and w[j][k] >= 1 and w[i][k] >= 1:
                    ...
                    for m in range(n):
                        if i<j:
                            if j<k:
                                if k<l:
                                    if l<m:
                                        if w[i][j] >= 1 and w[j][k] >= 1 and w[k][l] >= 1 and w[l][m] >=1 and w[i][m] >= 1:
